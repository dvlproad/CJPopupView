...
typedef void(^CJButtonStateChangeCompleteBlock)(CJRadioButton *CJRadioButton);

#define DEGREES_TO_RADIANS(angle) ((angle)/180.0 *M_PI)
#define RADIANS_TO_DEGREES(radians) ((radians)*(180.0/M_PI))

//采用UIImage＋UILabel + 手势的组合实现起来
@interface CJRadioButton : UIView {
    
}
@property (nonatomic, assign) CJButtonImagePosition imagePosition; /**< 图片的位置（必须设置,如果设为CJButtonImagePositionNone则不会添加图片） */

@property (nonatomic, copy) void (^tapAction)(CJRadioButton *cjButton);
